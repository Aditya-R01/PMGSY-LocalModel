...
# ----------------- Load and Cache All Models and Assets ------------------
@st.cache_resource
def load_assets():
    """Loads all required assets: the local RF model, its label encoder, and its columns."""
    try:
        with open('random_forest_model.pkl', 'rb') as f_model:
            local_model = pickle.load(f_model)

        with open('label_encoder.pkl', 'rb') as f_le:
            label_encoder = pickle.load(f_le)

        with open('model_columns.pkl', 'rb') as f_cols:
            model_columns = pickle.load(f_cols)

        return local_model, label_encoder, model_columns

    except FileNotFoundError as e:
        st.error("FATAL: A required file for the local fallback model was not found.")
        st.error("Please ensure 'random_forest_model.pkl', 'label_encoder.pkl', and 'model_columns.pkl' are present.")
        st.error(f"Details: {e}")
        return None, None, None


# Load all assets at the start
local_model, le, model_columns = load_assets()
...
# --- Feature names must match the order the model was trained on ---
FEATURE_NAMES = [
...
API_KEY = "0QfzNqqBLe5xJFxi6admmnnkPDL9TS9Z5H3KBi8jroGl"  # ⚠️ Replace with your actual API key
...
st.markdown("""
<style>
    /* Main app background with a bluish gradient */
    .stApp {
    ...
    .stTextInput>div>div>input, .stNumberInput>div>div>input, .stSelectbox>div>div {
        background-color: rgba(255, 255, 255, 0.05); color: #ffffff; border-radius: 8px; border: 1px solid #556070;
    }
    .stProgress > div > div > div > div { background-color: #00a6fb; }
    .stButton>button {
        color: #ffffff; background-color: #00a6fb; border-radius: 8px; border: 2px solid #00a6fb;
        font-weight: bold; transition: all 0.3s ease-in-out; padding: 10px 20px;
    }
    .stButton>button:hover { background-color: #ffffff; color: #00a6fb; transform: scale(1.05); }
    [data-testid="stMetricValue"] { font-size: 2.8em; color: #ffffff; font-weight: bold; }
    [data-testid="stMetricLabel"] { font-size: 1.3em; color: #bdc3c7; }
</style>
...
def display_results(label, confidence, source):
    st.success("✅ Classification Complete!")
    if source == "local":
        st.info("💡 Prediction was made using the local fallback RandomForest model.")
    elif source == "cloud":
        st.info("☁️ Prediction was made using the primary IBM Cloud model.")

    st.markdown("### Prediction Result:")
    res_col1, res_col2 = st.columns(2)
    with res_col1:
        st.metric(label="Project Class", value=label)
    with res_col2:
...
with col2:
    if st.button("🚀 Analyze and Classify Project", use_container_width=True):
        ...
        if all(value == 0 for value in all_numerical_inputs):
            st.warning("⚠️ Please enter at least one value for the project metrics.")
            st.stop()
        elif not district:
            ...
            with st.spinner("🧠 AI is processing the data..."):
                ...
                try:
                    ...
                    display_results(pred_label, confidence, source="cloud")

                except Exception as e:
                    # Fallback: Local model
                    st.warning(f"Cloud prediction failed. Reason: Monthly CUH Limit Exhausted.", icon="⚠️")
                    st.info("⚙️ Switching to local fallback model for prediction...")

                    if local_model is None or le is None or model_columns is None:
                        st.error("Cannot perform local prediction because one or more model asset files could not be loaded.")
                        st.stop()

                    # Prepare data for local model
                    input_data = {
                        'STATE_NAME': [state], 'DISTRICT_NAME': [district],
                        'NO_OF_ROAD_WORK_SANCTIONED': [no_road_san], 'LENGTH_OF_ROAD_WORK_SANCTIONED': [length_road_san],
                        'NO_OF_BRIDGES_SANCTIONED': [no_bridge_san], 'COST_OF_WORKS_SANCTIONED': [cost_work_san],
                        'NO_OF_ROAD_WORKS_COMPLETED': [no_road_comp], 'LENGTH_OF_ROAD_WORK_COMPLETED': [length_road_comp],
                        'NO_OF_BRIDGES_COMPLETED': [no_bridge_comp], 'EXPENDITURE_OCCURED': [exp_occurred],
                        'NO_OF_ROAD_WORKS_BALANCE': [no_road_bal], 'LENGTH_OF_ROAD_WORK_BALANCE': [length_road_bal],
                        'NO_OF_BRIDGES_BALANCE': [no_bridge_bal]
                    }
                    ...
                    prediction_encoded = local_model.predict(input_df_aligned)
                    prediction_proba = local_model.predict_proba(input_df_aligned)

                    prediction_decoded = le.inverse_transform(prediction_encoded)[0]
                    confidence = np.max(prediction_proba)

                    display_results(prediction_decoded, confidence, source="local")
